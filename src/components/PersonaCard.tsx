...
const PersonaCard = ({ user, variant = "default", showActions = true, customActions, onChatClick, onLikeClick, onShareClick, onAddFriendClick }: PersonaCardProps) => {
    ...
    <Card className={`group transition-all duration-300 hover:shadow-xl hover:scale-[1.03] ${isCompact ? 'p-3' : 'p-4'} border border-border/50 hover:border-primary/30 animate-fade-in bg-gradient-to-br from-card to-card/80 backdrop-blur-sm`}>
      <CardContent className={`${isCompact ? 'p-3' : 'p-6'} space-y-4`}>
        ...
        <div className="flex items-center gap-4">
          <div className={`${isCompact ? 'w-16 h-16' : 'w-20 h-20'} rounded-full bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center font-bold text-primary transition-all duration-300 group-hover:scale-110 group-hover:rotate-3`}>
        ...
        <div className="flex flex-wrap gap-2">
          {(showAllTags ? tags : tags.slice(0, maxTagsToShow)).map((tag, index) => (
            <Badge 
              key={tag} 
